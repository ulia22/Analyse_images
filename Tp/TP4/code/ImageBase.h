...
public:
	...
	int seuillageBinaire(unsigned char seuil);
	int eroder();
	int dilater();
	int ouvrir();
	int fermer();
	int ouvrirFermer6();
	int getVoisins(unsigned char* data, int i, unsigned char* res[]);
	int diff(ImageBase seuille, ImageBase dilate);
	void load(char *filename);
	bool save(char *filename);
	int toGrey(unsigned char dataGrey[]);
